...
public class LibDAO {
	...
	public void issueBook(Connection con) {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("insert into libmanage(book_id, stud_id, date_of_issues, due_date) values(?,?,?,?);");
	...
	public void recordReturn(Connection con)
	{
			...
			try
			{
				...
				if (rs3.next()) {
					...
					System.out.println("book_name::" + rs3.getInt(2));
